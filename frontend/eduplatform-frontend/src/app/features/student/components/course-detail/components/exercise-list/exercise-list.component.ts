...
export class ExerciseListComponent implements OnInit, OnChanges {
  ...
  @Output() editSubmission = new EventEmitter<{
    exercise: Exercise;
  ...
  getFilterCounts() {
    return {
      ...
      notSubmitted: this.exercises.filter((ex) => !this.getSubmission(ex))
        .length,
    };
  }
  ...
  /**
   * Verificar pistas de TODOS los ejercicios en paralelo con forkJoin
   */
  checkHintsAvailabilityOptimized() {
    if (this.exercises.length === 0) return;

    console.log(
      '🔍 Verificando pistas para',
    ...
    const hintRequests = this.exercises
      ...
      .map((exercise) => {
        ...
        return this.exerciseService.getHintsByExercise(exercise.id!).pipe(
          ...
          catchError((error) => {
            console.error(
              `❌ Error al verificar pistas para ejercicio ${exercise.id}:`,
              error
            );
            return of({
              exerciseId: exercise.id!,
    ...
    forkJoin(hintRequests).subscribe({
      next: (results) => {
        ...
        console.log(
          `✅ Verificación completada: ${totalWithHints}/${results.length} ejercicios tienen pistas`
        );
      },
      error: (error) => {
